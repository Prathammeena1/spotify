...
document.addEventListener('keydown', e => {
    const searchSongForPlaylist = document.querySelectorAll('.searchSongForPlaylist input') // Replace 'searchBar' with the actual ID of your search bar input element
        if (document.activeElement !== searchSongForPlaylist[0]) {
            ...
            // console.log('Space button pressed outside search bar');
        } else {
            // console.log('Space button pressed inside search bar');
    }
...
function volumeAdjust() {
    ...
    fetch(`/find/playlist/${playlistId}`).then(raw => raw.json()).then((data) => {
      ...
      if (data.music.length === 0) {
        ...
        data.music.forEach((song) => {
          console.log(song)
          songClutter += `<div class="songs bg-gradient-to-bl from-zinc-800 to-zinc-950 p-2 rounded-md flex items-center justify-between my-1">
              ...
              <div class="playlistImg h-8 w-8">
                ...
                <div>
                  <div class="songName text-sm text-zinc-400">
                    ...
                    <div>
                      <a class="deleteSong opacity-0" href="/deleteSongFromPlayliist/"><i class="ri-delete-bin-line text-zinc-400"></i></a>
                      </div>
                      </div>
                      </div>`
        })
  ...
  function addSongToPlayList(elem){
    console.log(elem.dataset.songId)
    const dataToSend = {
